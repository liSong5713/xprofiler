...
function getFinalUserConfigure(envConfig, userConfig) {
  ...
  const logDirIllegal = typeof finalConfig.log_dir === 'string' && !checkLogDirAccessiable(finalConfig.log_dir);
  let logDirMessage = '';
  if (logDirIllegal) {
    ...
    logDirMessage = `${finalConfig.log_dir} will be ignored (${extra}), use default log_dir: ${defaultConfig.log_dir}`;
    // output error log
