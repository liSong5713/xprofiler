...
function composeLogDirInfo(logdir) {
  return (
    [process.pid, logdir, process.cwd(), process.execPath, process.argv[1], path.join(__dirname, '..')].join(SPLITTER) +
    '\n'
  );
}
  ...
  if (fs.existsSync(dataFile)) {
    ...
    if (
      content
        .trim()
        .split('\n')
        .some((proc) => proc && Number(proc.split('\u0000')[0]) === process.pid)
    ) {
      shouldWrite = false;
    }
  }
